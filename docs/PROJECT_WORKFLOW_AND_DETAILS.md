# 🚚 Delivery Intelligence & Sequence Analytics — Complete Workflow & Technical Specification
...
### 5.5 Deep Learning Baseline: Gradient Boosted Trees vs. Deep Sequential DeliveryNet (PyTorch)
*Evaluates whether Deep Neural Networks outperform tree ensembles on point-in-time sequential dispatch features.*
...
> [!NOTE]
> **Key Scientific Takeaway**: Tree-based gradient boosting outperforms Deep Learning on point-in-time express logistics data ($+27.6\%\text{ higher } R^2$ and $+0.20\text{ higher ROC-AUC}$). Decision trees naturally partition non-linear spatial bounds and discrete shift resets without susceptibility to gradient vanishing over sparse categorical embeddings.
